  ...
  <div>
    <div>
      <q-card flat :bordered="!embedded">
        ...
        <q-form v-else ref="formRef" @submit.prevent="save">
          <q-card-section class="row q-col-gutter-sm q-px-none q-py-sm">
            ...
            <label-form class-name="col-12 col-md-4" text-label="Válido até"><q-input
              ...
              :rules="[validEndDate]"
            /></label-form>
            <q-checkbox v-model="form.isCurrent" label="Vínculo atual" class="q-mt-md" @update:model-value="onCurrentChange" />
          </q-card-section>
    ...
    <div v-if="!embedded" class="q-mt-md">
      <q-card flat :bordered="!embedded">
        ...
        <q-separator v-if="!embedded" />
        <q-card-section v-if="loading" class="row justify-center q-pa-lg"><q-spinner color="primary" /></q-card-section>
        <q-banner v-else-if="combinedError" class="bg-orange-1 text-warning">
          {{ combinedError }}
